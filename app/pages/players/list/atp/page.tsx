        ...
        <>
          ...
          <div className="mb-7 rounded-2xl border border-sky-500/20 bg-sky-500/5 overflow-hidden">
            ...
            <div className="px-5 pt-4 pb-3 flex items-start justify-between gap-3 border-b border-sky-500/10">
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-sky-400 mb-0.5">🏁 Race to Champions {new Date().getFullYear()}</p>
                <p className="text-xs text-slate-400">8 besar mungkin akan diadu kekuatan memperebutkan gelar di akhir tahun {new Date().getFullYear()}</p>
              </div>
                ...
                <div key={player.id} className="flex flex-col items-center gap-1">
                  <div className="relative">
                    {player.imgUrl ? (
                      ...
                      <div className="w-12 h-12 rounded-full bg-sky-500/10 border-2 border-sky-500/30 flex items-center justify-center">
                        <span className="text-sky-400 font-black text-base">{player.name.charAt(0).toUpperCase()}</span>
                      </div>
                    )}
                    <span className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full text-[10px] font-black flex items-center justify-center border-2 border-slate-950 bg-sky-500 text-white`}>
                      {player.rank}
                    </span>
                  </div>
