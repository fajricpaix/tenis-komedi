        ...
        <>
          ...
          <div className="mb-7 rounded-2xl border border-sky-500/20 bg-sky-500/5 overflow-hidden">
            ...
            <div className="px-5 pt-4 pb-3 flex items-start justify-between gap-3 border-b border-sky-500/10">
              <div>
                <p className="text-[10px] md:text-base font-black uppercase tracking-widest text-sky-400 mb-0.5">🏁 Race to Champions {new Date().getFullYear()}</p>
            ...
            <div className="p-4 grid grid-cols-4 gap-x-2 gap-y-4">
              {rankedPlayers.slice(0, 8).map((player) => (
                <div key={player.id} className="flex flex-col items-center gap-1">
                  <div className="relative">
                    {player.imgUrl ? (
                      <img src={player.imgUrl} alt={player.name}
                        className="w-12 md:w-24 h-12 md:h-24 rounded-full object-cover border-2 border-sky-500/40 shadow-md"
                      />
                    ) : (
                      <div className="w-12 md:w-24 h-12 md:h-24 rounded-full bg-sky-500/10 border-2 border-sky-500/30 flex items-center justify-center">
                        <span className="text-sky-400 font-black text-base md:text-4xl">{player.name.charAt(0).toUpperCase()}</span>
                      </div>
                    )}
                    <span className={`absolute -bottom-1 -right-1 w-5 md:w-8 h-5 md:h-8 rounded-full text-[10px] md:text-base font-black flex items-center justify-center border-2 border-slate-950 bg-sky-500 text-white`}>
                      {player.rank}
                    </span>
                  </div>
                  ...
                  <p className="text-[10px] md:text-sm font-black text-sky-400 tabular-nums">{(player.points * 100).toLocaleString()}</p>
                </div>
              ))}
            </div>
          </div>
